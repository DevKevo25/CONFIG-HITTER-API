...
class EHIDecryptor:
    ...
    def execute(cls, file_bytes: bytes) -> Optional[str]:
        if not ARGON2_AVAILABLE:
            return "❌ argon2-cffi not installed. Run: pip install argon2-cffi"

        payload = cls._parse_ehi_bytes(file_bytes)
        if not payload:
            return "❌ Failed to parse .ehi file"

        config, matched_iv = None, None

        # FIX: Use EHIConstants instead of cls
        ...
        if not config:
            return "❌ Failed to decrypt .ehi file"
        ...
        if matched_iv in EHIConstants.BYPASS_IVS:
            ...
            if not target_data or not (aaa_result := cls._decrypt_xor_layer(target_data, target_salt)):
                return "❌ Failed to decrypt config data"
            ...
            try:
                ...
                return f"❌ Argon2 decryption failed: {str(e)}"
