...
def build_dark(raw_bytes: bytes) -> Optional[bytes]:
    ...
    try:
        ...
        # Strip locks at outer msgpack level and re-encrypt
...
# Inner config fields per-item encrypted with L3_KEY + derived nonce
# ═══════════════════════════════════════════════════════════════════════════════
...
_SSC_L3    = bytes.fromhex("d39394517a48971f6e8555e994bee5bd835e5ab2f85fbd76bbd99800f32b967e")
_SSC_ENC_FIELDS = {"g", "h", "l", "o", "p", "v", "x", "i", "w"}
...
def _ssc_inner_nonce(user_key) -> Optional[bytes]:
    uk = str(user_key) if user_key else ""
    if len(uk) != 32:
        return None
    with contextlib.suppress(Exception):
        return bytes.fromhex(f"{uk[16:32][::-1]}68{uk[0:16]}")[:8]
    return None


def _ssc_reencrypt_configs(json_obj: dict) -> dict:
    """Re-encrypt per-config inner fields using L3_KEY after stripping locks."""
    ...
    for item in configs:
        # Strip lock fields
        item = _strip_locks(item)
        nonce = _ssc_inner_nonce(item.get("b", ""))
        if nonce:
            for field in _SSC_ENC_FIELDS.intersection(item.keys()):
                val = item[field]
                if isinstance(val, str) and val:
                    with contextlib.suppress(Exception):
                        plain = val.encode('utf-8') + b'\x00'
                        item[field] = _ssc_chacha(_SSC_L3, nonce, plain).hex()
        rebuilt.append(item)
    json_obj["a"] = rebuilt
    return json_obj
...
def build_ssc(raw_bytes: bytes) -> Optional[bytes]:
    ...
    try:
        ...
        if "c" in l1_json and isinstance(l1_json.get("a"), str):
            # Two-layer: decrypt L2, modify, re-encrypt L2
            ...
            l2_json = _ssc_reencrypt_configs(l2_json)
            ...
            l1_json = _ssc_reencrypt_configs(l1_json)
...
# ═══════════════════════════════════════════════════════════════════════════════
# HTTP CUSTOM  (.hc)
# Format: XOR(magic, hex(ChaCha20(KEY[5], STATIC_NONCE, outer_json)))
# Inner: ChaCha20(KEY[1], STATIC_NONCE, token_string)
# Tokens: individually encrypted ChaCha20(KEY[7], dyn_nonce) + JKL layer
# ═══════════════════════════════════════════════════════════════════════════════
...
# Token positions that control locking
...
def _hc_abc_encrypt(plaintext: str, key: bytes, nonce: bytes = _HC_STATIC_NONCE) -> str:
    """ChaCha20 encrypt plaintext → hex with 16 appended null bytes (mirrors _abc_decrypt)."""
    with contextlib.suppress(Exception):
        c = ChaCha20.new(key=key, nonce=nonce)
        c.seek(64)
...
def _hc_jkl_encrypt(plaintext: str, is_new: bool = False) -> str:
    ...
    with contextlib.suppress(Exception):
        ...
        for i, b in enumerate(inner_b64):
            k = key[i % 20]
            # Solve: b = _hc_f(outer[i]) ^ _hc_f(k)
            # _hc_f(outer[i]) = b ^ _hc_f(k)
            # outer[i] = _hc_f(b ^ _hc_f(k))  [since _hc_f is self-inverse]
...
def _hc_decrypt_token(token: str, dyn_nonce: bytes, is_new: bool) -> str:
    ...
    if is_new:
        ...
        # Old format: if hex, abc_decrypt with KEY[7] + nonce, then JKL
...
def _hc_encrypt_token(plain: str, is_new: bool) -> str:
    """Re-encrypt a plain token value using STATIC_NONCE (no lock meta → static nonce)."""
    if not plain:
        return ""
...
def build_hc(raw_bytes: bytes) -> Optional[bytes]:
    ...
    try:
        # Step 1: XOR with magic to reveal hex payload
        with contextlib.suppress(Exception):
        ...
        # Step 5: Process tokens — clear lock positions, re-encrypt rest with STATIC_NONCE
        ...
        new_xy = _hc_abc_encrypt(new_xy_dec, used_key)  # uses STATIC_NONCE (no lock meta)
        ...
        if is_new_format:
            ...
            new_outer.pop('xy', None)
            new_outer['a'] = new_obj_a
            new_outer['xy'] = new_xy

        # Step 7: Re-encrypt outer JSON and XOR with magic
        new_hex_payload = _hc_abc_encrypt(
            json.dumps(new_outer, ensure_ascii=False), _HC_CHACHA_KEYS[5])
        return _hc_xor(bytes.fromhex(new_hex_payload))
...
def _xxtea_encrypt(data: bytes, key: bytes) -> bytes:
    """XXTEA encryption (inverse of _xxtea_decrypt)."""
    actual_len = len(data)
    # Pack: data + padding + length as last uint32
    padded_blocks = (actual_len + 7) // 4  # uint32s needed (excluding length word)
...
def _ehi_parse_file(file_bytes: bytes):
    """Parse EHI binary → (header_bytes_before_payload, payload_bytes)."""
...
def build_ehi(raw_bytes: bytes) -> Optional[bytes]:
    ...
    try:
        ...
        matched_iv = None
        l2_nonce_used = None
        parts_1 = None  # middle part of l1_text (preserved for re-encryption)
        ...
        if is_bypass:
            # No inner configData encryption layer
            ...
            # Re-encrypt with a fresh nonce (same Argon2 key, same AAD)
            new_nonce = os.urandom(24)
            c4 = ChaCha20_Poly1305.new(key=argon_key, nonce=new_nonce)
            c4.update(aad)
            new_ciphertext, new_tag = c4.encrypt_and_digest(new_inner_bytes)

            new_raw_payload = aad + new_nonce + new_ciphertext + new_tag
        ...
        new_l1_text = f"{base64.b64encode(l2_nonce_used).decode()}:{parts_1}:{base64.b64encode(enc_parts2).decode()}"
...
# Cipher: whitebox CTR (encrypt_block used as keystream generator) — same for encrypt/decrypt
# ═══════════════════════════════════════════════════════════════════════════════
...
def build_npvt(raw_bytes: bytes) -> Optional[bytes]:
    ...
    try:
        ...
        with contextlib.suppress(Exception):
            ...
            result_json = parsed[0] if isinstance(parsed, list) and parsed else parsed

        # Strip lock flags
        result_json = _strip_locks(result_json)
        new_plain = json.dumps(result_json, ensure_ascii=False)
