# decryptors/ehi_cloud.py - Fixed with better error handling
...
class EHICloudDecryptor:
    ...
    def _decrypt_xor_layer(ciphertext_str: str, key: str) -> Optional[str]:
        ...
        with contextlib.suppress(Exception):
            hex_bytes_raw = EHICloudDecryptor._custom_b64_decode(ciphertext_str[::-1])
    ...
    def _decode_inner_fields(parsed_json: Dict[str, Any], salt_key: str) -> Dict[str, Any]:
        ...
        for k, v in parsed_json.items():
            if isinstance(v, str) and v.strip():
                decrypted_val = EHICloudDecryptor._decode_config_message(v) if k == "configMessage" else EHICloudDecryptor._decrypt_xor_layer(v, salt_key)
    ...
    @classmethod
    def fetch_and_decrypt(cls, url_or_key: str) -> Optional[Dict[str, Any]]:
        """Fetch EHI from cloud and decrypt"""
        ...
        if not url_or_key.startswith('http'):
            url = f"https://ehi.link/{url_or_key}"
        else:
            url = url_or_key
        
        # Validate URL
        if not url.startswith('https://ehi.link/'):
            return {'error': 'Invalid EHI cloud URL. Must be https://ehi.link/...'}
        ...
        try:
            ...
            if 'text/html' in content_type or response.text.startswith('<!DOCTYPE'):
                return {'error': 'The link returned HTML instead of an EHI file. The key may be invalid or expired.'}
            
            response.raise_for_status()
        except requests.exceptions.RequestException as e:
            logger.error(f'Failed to fetch EHI: {e}')
            return {'error': f'Failed to fetch from cloud: {str(e)}'}
        
        # Get the content
        file_bytes = response.content
        logger.info(f'Downloaded {len(file_bytes)} bytes from {url}')
        
        # Check if it's a valid EHI file
        if len(file_bytes) < 10:
            return {'error': 'Downloaded file is too small'}
        
        # Try to parse as EHI
        result = cls._decrypt_ehi_bytes(file_bytes)
        
        if result is None:
            return {'error': 'Decryption failed - file may be corrupted or not a valid EHI'}
    ...
    @classmethod
    def _decrypt_ehi_bytes(cls, file_bytes: bytes) -> Optional[str]:
        """Decrypt EHI bytes - same logic as EHIDecryptor.execute"""
...
def run(url_or_key: str) -> Optional[Dict[str, Any]]:
    """Entry point for EHI cloud decryption"""
    return EHICloudDecryptor.fetch_and_decrypt(url_or_key)